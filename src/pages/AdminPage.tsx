...
import { PRODUCTS, type ProductContentBlock, type ProductDefinition, type ProductSlug } from "../config/products";
import {
...
const AdminPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const loadedDrafts = buildDrafts(readProductOverrides(), tNo, tEn);
  ...
  function updatePageText(slug: ProductSlug, lang: "no" | "en", textKey: string, value: string) {
    setDrafts((current) => {
      if (!current) return current;
      ...
      return {
        ...
        [slug]: {
          ...
          pageTextNo:
            lang === "no"
              ? { ...current[slug].pageTextNo, [textKey]: value }
              : current[slug].pageTextNo,
          pageTextEn:
            lang === "en"
              ? { ...current[slug].pageTextEn, [textKey]: value }
              : current[slug].pageTextEn,
        },
      };
    });
  }
    ...
    <main className="page">
      <section className="fs-hero" style={{ maxWidth: 1100 }}>
        ...
        <p className="fs-tagline" style={{ maxWidth: 900 }}>
          Nå kan du styre både produktfliser og selve tekstinnholdet på produktsidene uten å inn i kode.
        </p>
        <p style={{ maxWidth: 900 }}>
          Denne versjonen lagrer fortsatt lokalt i nettleseren. Det er med vilje, så vi kan spikre struktur og arbeidsflyt før vi kobler på Firebase og ekte innlogging.
        </p>
                ...
                <div style={{ marginTop: "1.25rem", display: "grid", gap: "1.25rem" }}>
                  ...
                  <div>
                    ...
                    <p style={{ marginTop: 0, opacity: 0.8 }}>
                      Dette er første versjon. Du styrer alle tekstnøkler som brukes av denne produktsiden, på norsk og engelsk.
                    </p>
                    ...
                    <div style={{ display: "grid", gap: "1rem" }}>
                      {productTextKeys[product.slug].map((textKey) => (
                        ...
                        >
                          <p style={{ marginTop: 0, marginBottom: "0.35rem", fontWeight: 700 }}>
                            {prettifyKey(textKey)}
                          </p>
                          <p style={{ marginTop: 0, marginBottom: "0.85rem", opacity: 0.65 }}>
                            <code>{textKey}</code>
                          </p>
                          ...
                          >
                            <label style={{ display: "block" }}>
                              ...
                              <textarea
                                value={draft.pageTextNo[textKey] ?? ""}
                                onChange={(event) => updatePageText(product.slug, "no", textKey, event.target.value)}
                                rows={3}
                                style={{ width: "100%", padding: "0.7rem", resize: "vertical" }}
                              />
                            </label>
                            ...
                            <label style={{ display: "block" }}>
                              ...
                              <textarea
                                value={draft.pageTextEn[textKey] ?? ""}
                                onChange={(event) => updatePageText(product.slug, "en", textKey, event.target.value)}
                                rows={3}
                                style={{ width: "100%", padding: "0.7rem", resize: "vertical" }}
                              />
                            </label>
